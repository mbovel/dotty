...
// ----- Desugar methods -------------------------------------------------
   ...
   */
  private class DesugarQualifiedTypesMap(outerParamName: TermName) extends UntypedTreeMap:
    override def transform(tree: Tree)(using Context): Tree =
      tree match
        case QualifiedTypeTree(parent, None, qualifier) =>
          qualifiedType(transform(parent), outerParamName, qualifier, tree.span)
        case QualifiedTypeTree(parent, paramName, qualifier) =>
          cpy.asInstanceOf[UntypedTreeCopier].QualifiedTypeTree(tree)(transform(parent), paramName, qualifier)
