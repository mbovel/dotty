...
import dotty.tools.dotc.ast.tpd.{Apply, TypeApply, Ident, Literal, New, Select, Tree, ConstantTree, TreeMap, TypeTree, TreeOps}
  ...
  private def computeToNode(tree: Tree)(using Context): Option[ENode] =
    tree match
      ...
      case Apply(Select(clazz, nme.CONSTRUCTOR), args) if isCaseClass(clazz.symbol) =>
        for argsNodes <- args.map(toNode).sequence yield
          ENode.Object(clazz.symbol, argsNodes)
      case Select(qual, name) if isCaseClassField(tree.symbol) =>
        for qualNode <- toNode(qual) yield
          qualNode match
            case ENode.Object(_, args) => args(caseClassFieldIndex(tree.symbol))
            case qualNode => ENode.Select(qualNode, tree.symbol)
      case Apply(fun, args) =>
        for funNode <- toNode(fun); argsNodes <- args.map(toNode).sequence yield
          ENode.App(funNode, argsNodes)
      case TypeApply(fun, args) =>
        for funNode <- toNode(fun) yield
          ENode.TypeApp(funNode, args.map(_.tpe))
      case _ =>
        return None
