...
import dotty.tools.dotc.core.Hashable.BinderPairs
import dotty.tools.dotc.core.Hashable.Binders
import dotty.tools.dotc.core.Hashable.SomeBinderPairs
...
private abstract class QualifierComparer:
   ...
   *       instead of defining a different hash code for types.
   */
...
private[qualified_types] final class QualifierAlphaComparer(using Context) extends QualifierComparer:
  /** Two trees are the same if they are alpha-equivalent after normalization.
   */
  def same(tree1: Tree, tree2: Tree)(using Context) =
    trace(i"alpha same $tree1, $tree2"):
      iso(QualifierNormalizer.normalize(tree1), QualifierNormalizer.normalize(tree2))
