...
@main def aggregateData(dataDirString: String): Unit =
  ...
  for (benchmark, benchmarkRows) <- rows.flatten.groupBy(_.benchTime) do
...
def writeAggregatedRows(rows: collection.Seq[AggregatedRow], path: os.Path) =
  ...
  for row <- rows do
    writer.writeRow(Seq(row.benchmark, row.benchTime.toString(), row.commit, row.pr, row.min, row.q1, row.median, row.q3, row.max))
  writer.close()
