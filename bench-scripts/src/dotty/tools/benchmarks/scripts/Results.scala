...
case class Results(
  ...
  assert(importTime.getZone() == ZoneOffset.UTC, s"expected benchmark time '$importTime' to be in UTC")
  assert(importTime.isAfter(commitTime), s"expected benchmark time '$importTime' to be after commit time '$commitTime'")
