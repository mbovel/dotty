...
@main def importResults(
  ...
  val importTime = ZonedDateTime.now().withZoneSameInstant(ZoneOffset.UTC).withNano(0)
  importResults(pr, commit, merged, run, jmhOutput, dataJson, commitTime, importTime)
...
def importResults(
  ...
  println(s"importTime: $importTime")
  println(s"jmhOutput: $jmhOutput")
  println(s"dataJson: $dataJson")

  os.makeDir.all(dataJson / os.up)
  val results = Results(commitTime, commit, merged, pr, importTime, run, readJMHResults(jmhOutput))
