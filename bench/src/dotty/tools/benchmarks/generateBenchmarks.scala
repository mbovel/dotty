...
def generateBenchmarks(genDirName: String) =
  ...
  generateBenchmark("compiletime-ops", "sum-constants.scala", () =>
    val innerSum = (1 to 30)
      .map(i => f"$i")
      .mkString(" + ")
    val outerSum = (1 to 30)
      .map(_ => f"($innerSum)")
      .mkString(" + ")
    val vals = (1 to 20)
...
object Test:
  ...
  // Big compile-time sums of terms: (n + m + …). The result type is inferred.
  // The goal of this benchmark is to measure the performance cost of inferring
  // types for arithmetic operations.
  generateBenchmark("compiletime-ops", "sum-terms.scala", () =>
    val innerSum = (1 to 20)
      .map(_ => "one + m + n")
      .mkString(" + ")
    val outerSum = (1 to 10)
      .map(_ => f"($innerSum)")
      .mkString(" + ")
    val vals = (1 to 4)
      .map(i => f"val v$i = $outerSum")
...
object Test:
  ...
  def applicationCount = 8
  def applicationDepth = 8
