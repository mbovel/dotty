...
/** Compilation benchmarks.
  *
  * Descriptions of benchmarks are available in `bench/benchs.json`.
  *
  ...
  * and `mkdir` commands. It is only expected to work Unix-like systems.
  *
  ...
  * ```
  *
  * Benchmarks that should be run nightly contains `Nightly` in their benchmark
  * method name, and benchmarks that should be run using the bootstrapped
  * compiler contains `Bootstrapped`.
  *
  ...
  * ```
  *
  * To quickly test benchmarks, you can override the number of warmup and
  ...
  * ```
  *
  * Important: the `@Benchmark` methods, must be kept minimal to avoid
  * interference with the benchmarks.
  *
  * JMH examples:
...
class CompilationBenchmarks:
  ...
  def trialSetup(): Unit =
    ...
    // Generates benchmarks in `tests-generated`. Used to generate big (>500 kB)
    // Scala source files that are probably better generated on the fly than
