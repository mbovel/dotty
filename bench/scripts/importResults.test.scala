...
class ImporterSuite extends munit.FunSuite:
  ...
  test("readResultsExample1"):
    assertEquals(
      readJMHResults(os.Path("bench/scripts/tests/example_jmh_output_1.txt", os.pwd)),
      List(
  ...
  test("importResultsExample1"):
    ...
    Console.withOut(stdout) {
      importResults(
        ...
        // Assumes the working directory is the root of the repository
        "bench/scripts/tests/example_jmh_output_1.txt",
        outputFile.toString()
      )
    }
