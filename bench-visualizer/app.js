...
const benchmarks = {
  dotty: {
    title: "dotty (source changes over time)",
  },
  stdlib: {
    title: "scala stdlib-2.13",
  },
  scalap: {
    title: "scalap",
  },
  ...
  implicitNumsBootstrapped: {
    title: "implicit cache II - bootstrapped",
  },
  implicitNumsTasty: {
    title: "implicit cache II - from tasty",
  },
  implicitScopeLoop: {
    title: "implicit scope loop",
  },
  inductiveImplicits: {
    title: "inductive implicits",
  },
  findRef: {
    title: "Find Ref",
  },
  patmatExhaust: {
    title: "exhaustivity check",
  },
  exhaustivityI: {
    title: "exhaustivity I",
  },
  exhaustivityS: {
    title: "exhaustivity S",
  },
  exhaustivityT: {
    title: "exhaustivity T",
  },
  exhaustivityV: {
    title: "exhaustivity V",
  },
  patmatI7186: {
    title: "exhaustivity MIPS",
  },
  patmatI12241: {
    title: "exhaustivity i12241",
  },
  patmatI12358: {
    title: "exhaustivity i12358",
  },
  posI13565: {
    title: "exhaustivity i13565",
  },
  i1535: {
    title: "issue #1535",
  },
  ...
  powerMacro1: {
    title: "Inline a quote",
  },
  powerMacro1k: {
    title: "Inline 1k quotes",
  },
  stringInterpolationMacro: {
    title: "Quote String interpolation matching",
  },
  tuple22CreationApply: {
    title: "Tuple22 creation with Tuple22.apply",
  },
  tuple22CreationCons: {
    title: "Tuple22 creation with *:",
  },
  tuple22Tails: {
    title: "Tuple22.tail",
  },
  ...
  compiletimeSumConstants: {
    title: "Compile-time sums of constant integer types (generated)",
  },
  compiletimeSumTermrefs: {
    title: "Compile-time sums of term reference types (generated)",
  },
  compiletimeSumTerms: {
    title: "Sums of term references, result type inferred (generated)",
  },
  compiletimeSumApplications: {
    title: "Compile-time sums of type applications (generated)",
  },
  compiletimeDistribute: {
    title: "Compile-time additions inside multiplications (generated)",
  },
};

const main = selectOrThrow("main");

const dataUrl = "https://dotty-bench.epfl.ch/v3/data";
...
/**
 * Options for the aggregated view. See the HTML documentation inside
 ...
 * DOM (or have been scheduled to be created) before calling this function. The
 * graphs are rendered asynchronously.
 *
 * @param {AggregatedOptions} options
 * @returns {void}
 */
function renderAggregatedGraphs(options) {
  for (const id of Object.keys(benchmarks)) {
    setTimeout(() => renderAggregatedGraph(id, options));
  }
}
...
async function renderAggregatedGraph(id, options) {
  ...
  if (options.spaceEvenly) {
    ...
    const labelInterval = Math.ceil(indices.length / 5);
 ...
 * memoized.
 *
 * @type {(id: string, filter: "all" | "last100") => Promise<AggregatedData>} Aggregated data columns.
 */
const loadAggregatedData = memo(
  async function (id, filter) {
    console.log(`Loading aggregated data for ${id} with filter ${filter}`);
    const path = `${dataUrl}/aggregated/${filter}/${id}.csv`;
    const response = await fetch(path);

    if (!response.ok) {
      return { indices: [], times: [], commits: [], prs: [], mins: [], medians: [], maxs: [], movingAverage: [] };
    }
    ...
    for (const line of data.split("\n")) {
      const [time, commit, pr, min, q1, median, q3, max] = line.split(",");
      if (line === "") continue;
      times.push(time);
      commits.push(commit);
      prs.push(pr);
 ...
 *
 * @type {(commitHash: string) => Promise<DetailedData>}
 */
const loadDetailedData = memo(
  async function (commit) {
    const res = new Map();
    try {
      const indexResponse = await fetch(`${dataUrl}/raw/${commit}/index.csv`);
      const indexData = await indexResponse.text();
      const profileRunPairs = indexData
        .split("\n")
        .filter((line) => line !== "")
        .map((line) => {
          const [profile, run] = line.trim().split(",");
          return { profile, run };
        });

      for (const { profile, run } of profileRunPairs) {
        const response = await fetch(`${dataUrl}/raw/${commit}/${profile}-${run}.json`);
        const data = await response.json();
        for (const results of data.benchResults) {
          res.set(results.benchmark, (res.get(results.benchmark) ?? []).concat(results.measures));
        }
      }
    } catch (error) {
      console.error(`Failed to load detailed data for commit ${commit}:`, error);
    }
    return res;
  },
  ([commit]) => commit
 ...
 * @template A
 * @template R
 * @param {(a: A) => R} fn Function to memoize
 * @param {(a: *) => *} keyFn Function to generate the cache key from the arguments
 * @returns {(a: A) => R} Memoized function
 */
